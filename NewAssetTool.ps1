...
function Add-DepartmentOption([string]$value){
  if([string]::IsNullOrWhiteSpace($value)){ return }
  $raw = [string]$value
  $norm = Normalize-Field $raw
  if([string]::IsNullOrWhiteSpace($norm)){ return }
  if(-not $script:DepartmentListNorm.Contains($norm)){
    [void]$script:DepartmentList.Add($raw.Trim())
    [void]$script:DepartmentListNorm.Add($norm)
  }
}
function Get-DepartmentValuesFromFile([string]$path){
  $values = New-Object System.Collections.Generic.List[string]
  if([string]::IsNullOrWhiteSpace($path) -or -not (Test-Path $path)){ return $values }
  $lines = @()
  try { $lines = Get-Content -Path $path } catch { return $values }
  if(-not $lines -or $lines.Count -eq 0){ return $values }
  $firstNonEmpty = $null
  foreach($line in $lines){
    if(-not [string]::IsNullOrWhiteSpace($line)){ $firstNonEmpty = $line; break }
  }
  $rows = @()
  $treatAsHeader = $false
  if($firstNonEmpty){
    $probe = $firstNonEmpty.Trim().Trim([char]34)
    if($probe.ToUpperInvariant() -eq 'DEPARTMENT'){ $treatAsHeader = $true }
  }
  if($treatAsHeader){
    try { $rows = Import-Csv -Path $path } catch { $rows = @() }
  } else {
    try { $rows = ConvertFrom-Csv -InputObject $lines -Header 'Department' } catch { $rows = @() }
  }
  foreach($row in $rows){
    if(-not $row){ continue }
    $props = $row.PSObject.Properties | Where-Object { $_.Value -and -not [string]::IsNullOrWhiteSpace([string]$_.Value) }
    if(-not $props){ continue }
    $prop = $props | Where-Object { $_.Name -eq 'Department' } | Select-Object -First 1
    if(-not $prop){ $prop = $props | Select-Object -First 1 }
...
function Load-DepartmentMaster(){
  try{
    ...
    if(Test-Path $fileD){
      $script:DepartmentMaster = Get-DepartmentValuesFromFile $fileD
      foreach($val in $script:DepartmentMaster){ Add-DepartmentOption $val }
    }
    ...
    if(Test-Path $fileDU){
      $script:DepartmentUserAdds = Get-DepartmentValuesFromFile $fileDU
      foreach($val in $script:DepartmentUserAdds){ Add-DepartmentOption $val }
    }
...
function Save-DepartmentUserAdd([string]$dept){
  try{
    ...
    if($script:DepartmentListNorm -and $script:DepartmentListNorm.Contains($n)){ return }
    $fileDU = $null; try { if($script:DataFolder){ $fileDU = Join-Path $script:DataFolder 'DepartmentMaster-UserAdds.csv' } } catch {}
    if(-not $fileDU -or -not (Test-Path $fileDU)){
      try { $fileDU = Join-Path $PSScriptRoot 'Data/DepartmentMaster-UserAdds.csv' } catch {}
    }
