...
$tlpAssocStrip.Dock = 'None'
$tlpAssocStrip.Anchor = 'Right'
$tlpAssocStrip.Margin = '0,6,0,0'
...
function Get-ProposedParentToken($cand,$parentRec){
  ...
  $parentHost = $null
  if($parentRec.name){ $parentHost = $parentRec.name.Trim() }
  if([string]::IsNullOrWhiteSpace($parentHost)){ $parentHost = $parentRec.asset_tag }
  return $parentHost
}
function Get-PeripheralLinkPreview($cand,$parentRec){
  if(-not $cand){ return [pscustomobject]@{ ProposedName=''; ProposedParent='' } }
  $previewParent = Get-ProposedParentToken $cand $parentRec
  $proposedName = Compute-ProposedName $cand $parentRec
  return [pscustomobject]@{ ProposedName=$proposedName; ProposedParent=$previewParent }
}
...
function Show-AddPeripheralDialog($parentRec){
  ...
  $tblPreview.SetColumnSpan($lblTypeVal,3)
  $tblPreview.Controls.Add((& $createLabel 'Name:'),0,1)
  $tblPreview.Controls.Add($lblNameVal,1,1)
  $tblPreview.Controls.Add($lblNameArrow,2,1)
  $tblPreview.Controls.Add($lblProposedNameVal,3,1)
  $tblPreview.Controls.Add((& $createLabel 'Current Parent:'),0,2)
  $tblPreview.Controls.Add($lblParentVal,1,2)
  $tblPreview.Controls.Add($lblParentArrow,2,2)
  $tblPreview.Controls.Add($lblProposedParentVal,3,2)
  $tblPreview.Controls.Add((& $createLabel 'Asset Tag:'),0,3)
  $tblPreview.Controls.Add($lblAssetVal,1,3)
  $tblPreview.SetColumnSpan($lblAssetVal,3)
  $tblPreview.Controls.Add((& $createLabel 'Serial Number:'),0,4)
  $tblPreview.Controls.Add($lblSerialVal,1,4)
  $tblPreview.SetColumnSpan($lblSerialVal,3)
  $tblPreview.Controls.Add((& $createLabel 'RITM:'),0,5)
  $tblPreview.Controls.Add($lblRITMVal,1,5)
  $tblPreview.SetColumnSpan($lblRITMVal,3)
  $tblPreview.Controls.Add((& $createLabel 'Retire:'),0,6)
  $tblPreview.Controls.Add($lblRetireVal,1,6)
  $tblPreview.SetColumnSpan($lblRetireVal,3)

  $lblNameArrow.Margin = '6,0,6,4'
  $lblParentArrow.Margin = '6,0,6,4'
  ...
  $applyPreview = {
    ...
    $setArrow = {
      param($arrowLabel,$proposedLabel,[string]$proposedText)
      $arrowLabel.Text = ''
      $proposedLabel.Text = ''
      if(-not [string]::IsNullOrWhiteSpace($proposedText)){
        $arrowLabel.Text = ' ----> '
        $proposedLabel.Text = $proposedText
      }
    }
    if($preview){
      & $setArrow $lblNameArrow $lblProposedNameVal $preview.ProposedName
      & $setArrow $lblParentArrow $lblProposedParentVal $preview.ProposedParent
    } else {
      & $setArrow $lblNameArrow $lblProposedNameVal $null
      & $setArrow $lblParentArrow $lblProposedParentVal $null
    }
    if([string]::IsNullOrWhiteSpace($lblProposedParentVal.Text) -and -not [string]::IsNullOrWhiteSpace($lblParentVal.Text)){
      & $setArrow $lblParentArrow $lblProposedParentVal $lblParentVal.Text
    }
