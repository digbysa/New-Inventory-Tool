...
try { $form.Cursor = [System.Windows.Forms.Cursors]::WaitCursor } catch {}
  ...
  foreach ($pc in $script:Computers) {
    ...
    $maintenanceValue = Get-MaintenanceTypeOrDefault $mtRaw ([string]$pc.name)
    $isCritical = $false
    if (-not [string]::IsNullOrWhiteSpace($maintenanceValue)) {
      if ($maintenanceValue.Trim() -eq 'Critical Clinical') { $isCritical = $true }
    }
    ...
    if (-not $chkCriticalClinical.Checked -and $isCritical) { continue }
    ...
    $r.Cells['MaintenanceType'].Value = $maintenanceValue
